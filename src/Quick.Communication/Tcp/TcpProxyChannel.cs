...
namespace Quick.Communication
{
    ...
    public class TcpProxyChannel
    {
        ...
        private void _server_MessageReceived(object sender, MessageReceivedEventArgs args)
        {
            if (!IsRunning)
            {
                return;
            ...
            if (_clientDict.ContainsKey(proxyClientID))
            {
                if (DataFilter != null)
                {
                    DataFilter.BeforeClientToServer(new TcpRawMessage() { ClientID = args.ClientID, MessageRawData = args.MessageRawData });
                }
                try
                {
                    //the method may throw exception when stopping, so try catch is used but do nothing.
                    _clientDict[proxyClientID].SendMessage(args.MessageRawData.ToArray());
                }
                catch { }
            }
        }

        private void TcpClient_MessageReceived(object sender, MessageReceivedEventArgs args)
        {
            if (!IsRunning)
            {
                return;
            ...
            if (DataFilter != null)
            {
                DataFilter.BeforeServerToClient(new TcpRawMessage
                {
                    ...
                    MessageRawData = args.MessageRawData
                });
            }
            try
            {
                //the method may throw exception when stopping, so try catch is used but do nothing.
                _server.SendMessage(proxyClientID, args.MessageRawData.ToArray());
            }
            catch { }
        }
